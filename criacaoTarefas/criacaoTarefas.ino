...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    xTaskCreate(vTarefaSecundaria, NULL, TAM_PILHA_TS, NULL, PRIORIDADE_TS, &xTask);
    fim = micros();     //Salva o tempo depois da execução da função de criação do recurso
    if(xTask != NULL){               //Condição para verificar se o recurso foi criado
