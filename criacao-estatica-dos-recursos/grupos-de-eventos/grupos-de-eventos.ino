...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    StaticEventGroup_t xArmazenamentoGrupoDeEventos;
    ...
    xGrupoDeEventos = xEventGroupCreateStatic(&xArmazenamentoGrupoDeEventos);;
    fim = micros();     //Salva o tempo depois da execução da função de criação do recurso
    if(xGrupoDeEventos != NULL){ //Caso o recurso tenha sido devidamente criado:
      ...
      vEventGroupDelete(xGrupoDeEventos);
