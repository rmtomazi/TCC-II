...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    StaticSemaphore_t xBufferDoSemaforoBinario;
    ...
    xSemaforoBinario = xSemaphoreCreateBinaryStatic(&xBufferDoSemaforoBinario);
    fim = micros();     //Salva o tempo depois da execução da função de criação do recurso
    if(xSemaforoBinario != NULL){ //Caso o recurso tenha sido devidamente criado:
      ...
      vSemaphoreDelete(xSemaforoBinario);
