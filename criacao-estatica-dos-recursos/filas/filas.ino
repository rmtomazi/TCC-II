...
#define PILHA_TP        600   //Pilha da tarefa principal
...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    static StaticQueue_t xFilaEstatica;
    uint8_t ucAreaArmazenamentoFila[TAM_FILA * TAM_ITEM];
    ...
    xFila = xQueueCreateStatic(TAM_FILA, TAM_ITEM, ucAreaArmazenamentoFila, &xFilaEstatica);
    fim = micros();     //Salva o tempo depois da execução da função de criação do recurso

    if(xFila != NULL){ //Caso o recurso tenha sido devidamente criado:
