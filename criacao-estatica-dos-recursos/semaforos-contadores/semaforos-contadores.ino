...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    StaticSemaphore_t xBufferDoSemaforoContador;
    ...
    xSemaforoContador = xSemaphoreCreateCountingStatic(1, 0, &xBufferDoSemaforoContador);
    fim = micros();     //Salva o tempo depois da execução da função de criação do recurso
    if(xSemaforoContador != NULL){ //Caso o recurso tenha sido devidamente criado:
      ...
      vSemaphoreDelete(xSemaforoContador);
