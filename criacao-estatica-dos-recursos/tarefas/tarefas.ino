...
#define PILHA_TP        700   //Pilha da tarefa principal
#define PRIORIDADE_TP   3     //Prioridade da tarefa principal

#define PILHA_TS        512   //Pilha da tarefa secundária, que será criada
...
void vTarefaPrincipal(void *){
  ...
  do{
    StaticTask_t xBufferDaTarefa;
    StackType_t xPilha[PILHA_TS];
    TaskHandle_t xTarefa = NULL;
    ...
    xTarefa = xTaskCreateStatic(vTarefaSecundaria, "", PILHA_TS, NULL, PRIORIDADE_TS, xPilha, &xBufferDaTarefa);
    fim = micros();     //Salva o tempo depois da execução da função de criação do recurso

    if(xTarefa != NULL){ //Caso o recurso tenha sido devidamente criado:
