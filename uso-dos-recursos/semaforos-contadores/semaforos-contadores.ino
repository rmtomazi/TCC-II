...
void setup(){
  ...
  xSemaforoContador =  xSemaphoreCreateCounting(1, 1);
 ...
 * contador. Essa tarefa irá executar a função de aquisição e entrega do semáforo várias 
 * vezes e calcular a média do tempo de execução de cada função.
 */
...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    inicio = micros();  //Salva o tempo antes da execução da função de aquisição do semáforo
    xSemaphoreTake(xSemaforoContador, portMAX_DELAY);
    fim = micros();     //Salva o tempo depois da execução da função de aquisição do semáforo
    mediaTempoAquisicao += float(fim - inicio);
    inicio = micros();  //Salva o tempo antes da execução da função de entrega do semáforo
    xSemaphoreGive(xSemaforoContador);
    fim = micros();     //Salva o tempo depois da execução da função de entrega do semáforo
