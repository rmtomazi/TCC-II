...
SemaphoreHandle_t xSemaforoBinario;
 ...
 * usando semáforos binário. Essa tarefa irá aguardar pelo sinal, gerado pela rotina do
...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    xSemaphoreTake(xSemaforoBinario, portMAX_DELAY); //Aguarda um sinal no xSemafaroBinario
    somaTempo += micros();
  }while(i--);
...
static void ulTrataInterrupcaoISR(void){
    ...
    somaTempo -= micros();
    //Envia um sinal ao xSemaforoBinario
    xSemaphoreGiveFromISR(xSemaforoBinario, &xHigherPriorityTaskWoken);
}
