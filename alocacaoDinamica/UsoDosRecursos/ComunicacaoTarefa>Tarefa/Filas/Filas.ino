 ...
 * fila. Essa tarefa irá colocar uma mensagem na fila1. A tarefa secundária estará aguardando
 * por essa mensagem e ao receber irá adicionar a mesma na fila2, fila essa que a tarefa 
...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    xQueueSend(fila1, mensagem, portMAX_DELAY);     //Envia a mensagem para a tarefa secundária
    xQueueReceive(fila2, mensagem, portMAX_DELAY);  //Aguarda pela mensagem da tarefa secundária
  }while(i--);
 ...
 * pela fila1. Ao receber a mensagem ela irá ser desbloqueada e também enviará uma mensagem
 * idêntica à tarefa principal pela fila2. Ficando nesse laço por tempo indeterminado.
 */
void vTarefaSecundaria(void *){
  do{
    //Recebe o recurso e devolve para a tarefa principal
    xQueueReceive(fila1, mensagem, portMAX_DELAY);  //Aguarda pela mensagem da tarefa primária
    xQueueSend(fila2, mensagem, portMAX_DELAY);     //Envia a mensagem para a tarefa primária
  }while(1);
}
