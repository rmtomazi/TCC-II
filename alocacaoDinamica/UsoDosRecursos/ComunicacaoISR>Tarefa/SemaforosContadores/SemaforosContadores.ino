...
void setup(){
  ...
  xSemaforoContador =  xSemaphoreCreateCounting(1, 0);
  ...
  //Criação a tarefa secundária, que gerará a interrupção
 ...
 * usando semáforos contadores. Essa tarefa irá aguardar pelo sinal, gerado pela rotina do
...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    xSemaphoreTake(xSemaforoContador, portMAX_DELAY);  //Aguarda um sinal no xSemafaroContador
    somaTempo += micros();
  }while(i--);
...
static void ulTrataInterrupcaoISR(void){
    ...
    somaTempo -= micros();
    //Envia um sinal ao xSemaforoContador
