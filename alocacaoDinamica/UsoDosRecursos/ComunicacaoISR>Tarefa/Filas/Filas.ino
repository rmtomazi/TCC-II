...
void setup(){
  ...
  xFila = xQueueCreate(1, TAM_MENSAGEM);
 ...
 * usando filas. Essa tarefa irá aguardar pelo sinal, gerado pela rotina do
...
void vTarefaPrincipal(void *){
  ...
  do{
    ...
    xQueueReceive(xFila, mensagem, portMAX_DELAY);  //Aguarda pela mensagem do ISR
    somaTempo += micros();
  }while(i--);
...
static void ulTrataInterrupcaoISR(void){
    ...
    xQueueSendToBackFromISR(xFila, mensagem, &xHigherPriorityTaskWoken); 
}
